...
def create_calendar_html(df_month, year, month):
    ...
    for i, w in enumerate(weekdays):
        html += f'<div class="cal-header" style="color:{colors[i]};">{w}</div>'
    
    for week in month_days:
        for day in week:
            if day == 0:
                html += '<div class="cal-cell" style="border:none; background:transparent; cursor:default;"></div>'
            else:
...
def show_dashboard():
    ...
    with tab_map["📅 カレンダー"]:
        if not df_month.empty:
            # 1. Render Grid
            cal_html = create_calendar_html(df_month, st.session_state.view_date.year, st.session_state.view_date.month)
            components.html(cal_html, height=450, scrolling=True) # Reduced height since details are outside
            
            # 2. Render Details (Native)
            if 'selected_cal_date' in st.session_state:
                sel_date = st.session_state['selected_cal_date']
                try:
                    # Check if selected date belongs to current view
                    sy, sm, _ = map(int, sel_date.split('/'))
                    if sy == st.session_state.view_date.year and sm == st.session_state.view_date.month:
